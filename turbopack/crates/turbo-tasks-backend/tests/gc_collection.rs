...
/// invalidator behind, and the next mutation walks the list and opens each one `MustExist`.
///
/// Invalidators are weak references: a reader that no longer exists needs no invalidation, so the
/// mutation must skip it rather than panic with "task exists in neither memory nor persistent
/// storage".
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
