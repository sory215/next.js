...
///
/// Only the thread that performed its own restore I/O can conclude the task exists nowhere, and
/// only that thread still holds the entry — so only it can remove the entry it had to materialize.
/// A guard obtained by waiting on another thread's restore proves the task exists, so it never
/// needs to.
enum OpenedTask<'a> {
...
/// `MaybeCreate`, [`ExecuteContext::try_get_task`] = `Weak`).
...
enum TaskAccess {
    ...
    MustExist,
    /// Open a task the caller holds only a *weak* reference to: it may legitimately have been
    /// collected. Yields `None` when the task exists nowhere, and also when it is soft-deleted but
    /// still resident — from a weak holder's perspective those are the same state, and only the
    /// timing of the next eviction separates them.
    Weak,
}
...
pub trait ExecuteContext<'e>: Sized {
    ...
    /// Opens a task the caller holds only a **weak** reference to, returning `None` if it is gone.
    ///
    /// Gone covers both a task that exists nowhere and one that is soft-deleted: a weak holder
    /// cannot tell those apart, since only the timing of the next eviction separates them.
    ///
    /// Use this only where a stale reference is expected rather than a bug. Elsewhere prefer
    /// [`Self::task`], which panics instead of hiding graph corruption.
    fn try_get_task(
        &mut self,
        task_id: TaskId,
...
impl<'e> ExecuteContextImpl<'e> {
    ...
    fn open_task(
        ...
        // A soft-deleted task is gone as far as a weak holder is concerned: GC has unlinked it and
        ...
        if access == TaskAccess::Weak && task.flags.deleted() {
            self.task_lock_counter.release();
            return None;
        }
        let maybe_fabricated = matches!(access, TaskAccess::MustExist | TaskAccess::Weak)
        ...
        if !task.flags.is_restored(category) {
            if task_id.is_transient() {
                ...
                if do_data || do_meta || data_restoring || meta_restoring {
                    ...
                    // else was restoring it): it exists nowhere. A `Weak` open reports that; a
                    // `MustExist` open fails loudly rather than hand back a fabricated task, which
...
impl<'e> ExecuteContext<'e> for ExecuteContextImpl<'e> {
    ...
    fn try_get_task(
        ...
        self.open_task(task_id, category, TaskAccess::Weak)
    }
